...
//         let cluster = contract
//             .cluster_get(status.bucket.cluster_id)
//             .unwrap()
//             .cluster;

//         for v_nodes in cluster.v_nodes {
//             // Check that this bucket is allocated in the storage cluster of this node.
//             let allocated = v_nodes.contains(&(self.node.node_id as u64));
...
//                 assert!(
//                     authorized,
//                     "sender is not authorized to write to this bucket"
//                 );
...
//                     .stored_data
//                     .iter()
//                     .rfind(|row| row.bucket_id == bucket_id && row.routing_key == routing_key)
