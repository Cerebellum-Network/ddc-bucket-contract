...
fn cdn_node_create_err_if_node_exists() {
    ...
    assert_eq!(
        ctx.contract
            .cdn_node_create(ctx.cdn_node_key1, ctx.cdn_node_params1,),
        Err(CdnNodeAlreadyExists)
    );
}

#[ink::test]
fn cdn_node_create_ok() {
    let mut ctx = setup_cluster();
...
fn cdn_node_set_params_err_if_not_provider() {
    ...
    assert_eq!(
        ctx.contract
            .cdn_node_set_params(ctx.cdn_node_key0, new_cdn_node_params),
        Err(OnlyCdnNodeProvider)
    );
}
...
fn node_set_params_ok() {
    ...
    ctx.contract
        .cdn_node_set_params(ctx.cdn_node_key0, new_cdn_node_params.clone())?;
...
fn cdn_node_get_err_if_node_does_not_exist() {
    ...
    let bad_cdn_node_key = AccountId::from([
