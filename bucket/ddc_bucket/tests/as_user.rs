...
// use super::env_utils::*;
// use super::node::{find_cluster, Action, TestRequest};
...
//         let bucket_id =
//             contract.bucket_create(BUCKET_PARAMS.to_string().unwrap(), cluster_id, None);
//         pop_caller();
...
//             "should find the bucket that we created before"
//         );

//         // Find a gateway cluster.
//         let cdn_cluster = find_cluster(contract, GATEWAY_ENGINE)?.cluster;
//         // Pick a gateway node.
//         let _cdn_node_id = *cdn_cluster.v_nodes.first().expect("empty cluster");
//         let cdn_node = contract.node_get(0, 0)?;
