...
impl DdcBucket {
    ...
    pub fn message_cdn_node_create(&mut self, cdn_node_key: CdnNodeKey, cdn_node_params: CdnNodeParams) -> Result<CdnNodeKey> {
        let provider_id = Self::env().caller();

        let undistributed_payment: Balance = 0;

        let cdn_node_key = self
            .cdn_nodes
            .create(cdn_node_key, provider_id, cdn_node_params.clone(), undistributed_payment)
            .unwrap();
    ...
    pub fn message_cdn_node_change_params(&mut self, cdn_node_key: CdnNodeKey, cdn_node_params: CdnNodeParams) -> Result<()> {
        let caller = Self::env().caller();

        let mut cdn_node = self.cdn_nodes.get(cdn_node_key)?;
        cdn_node.only_owner(caller)?;
        cdn_node.cdn_node_params = cdn_node_params;
